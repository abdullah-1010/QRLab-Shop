...
const PaymentComponent = ({ priceData, stripeKey }) => {
  ...
  const handlePayament = async (token) => {
    setIsLoading(true);
    await fetch(`${API_URL}/pay?amount=${paymentInfo()}`, {
      method: "POST",
      mode: "cors",
      headers: {
  ...
  async function paymentSubmitHandler(e) {
    ...
    try {
      ...
      const client_secret =
        "sk_test_51OVNtfJYn5raN1M8xEYMzJBWQ9HPesPIJVFukhUDFcZRDr5069e70mqUQx4z1buMMAWVsBlOCTsbMuks0YZhjetL00SAyzKLmy";
      ...
      const result = await stripe.confirmCardPayment(client_secret, {
        payment_method: {
          ...
          billing_details: {
            name: priceData.recipient?.name,
            email: priceData?.recipient?.email,
            address: {
              line1: priceData?.recipient?.address1,
              state: priceData?.recipient?.state_code,
              postal_code: priceData?.recipient?.zip,
              country: priceData?.recipient?.country_code,
            },
          },
        },
      });
              ...
              <div className="order_summary">
                ...
                <div className="order_summary_details">
                  <div className="price order_Summary_Item">
                    ...
                    <p>
                      <b>{dispalyMoney(cartItems[0]?.price)}</b>
                    </p>
                  </div>
